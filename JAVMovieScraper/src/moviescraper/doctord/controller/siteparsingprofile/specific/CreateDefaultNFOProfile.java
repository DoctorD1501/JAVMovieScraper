...
// madaustrian added
//
// I derived this parser from Caribbeancom parser because Caribbeancom initializes the movie in such a way, 
// that it can be used further for writing a .NFO file

public class CreateDefaultNFOProfile extends SiteParsingProfile implements
		SpecificProfile {
	...
	private void initializeJapaneseDocument()
	{
		if(document != null && japaneseDocument == null)
		{
			...
			String url = "http://javseeds.com/" + id + "/index.html";
	...
	public Rating scrapeRating() {
		initializeJapaneseDocument();
		Element stars = japaneseDocument.select("div.movie-info dl dt:contains(ãƒ¦ãƒ¼ã‚¶ãƒ¼è©•ä¾¡:) ~ dd ").first();
		if(stars != null && stars.text().contains("â˜…"))
		{
			//count the number of â˜… characters, max number of stars is 5 and half stars not supported
	...
	public ReleaseDate scrapeReleaseDate() {
		initializeJapaneseDocument();
		Element releaseDate = japaneseDocument.select("div.movie-info dl dt:contains(é…�ä¿¡æ—¥:) ~ dd ").first();
	...
	public Runtime scrapeRuntime() {
		initializeJapaneseDocument();
		Element durationElement = japaneseDocument.select("div.movie-info dl dt:contains(ï¿½?ç”Ÿæ™‚é–“:) ~ dd ").first();
...
//		Elements genres = japaneseDocument.select("div.movie-info dl.movie-info-cat:contains(ã‚«ãƒ†ã‚´ãƒªãƒ¼:) dd ");
//		if(genres != null)
//		{
...
//		}
		...
		genreList.add(new Genre("JAV"));
//		genreList.add(new Genre("censored"));
//		genreList.add(new Genre("Bondage"));
//		genreList.add(new Genre("Urination"));
	...
	public ArrayList<Actor> scrapeActors() {
		...
		Elements japaneseActors = japaneseDocument.select("div.movie-info dl dt:contains(å‡ºæ¼”:) ~ dd a");
	...
	public String createSearchString(File file) {
		...
		String englishPage = "http://javseeds.com/";
		return englishPage;
	}
	...
	public SiteParsingProfile newInstance() {
		...
		return new CreateDefaultNFOProfile();
